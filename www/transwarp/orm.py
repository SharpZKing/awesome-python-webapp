...
class Filed(object):
	...
	def __init__(self,**kw):
		...
		self.ddl = kw.get('ddl','')
		self._order = Filed._count
		Filed._count = Filed._count + 1
...
def _gen_sql(table_name,mappings):
	...
	for f in sorted(mappings.values(),lambda x,y:cmp(x._order, y._order)):
		if not hasattr(f,'ddl'):
			raise StandardRrror('no ddl in field "%s".' %n)
		ddl = f.ddl
		nullable = f.nullable
		if f.primary_key:
			pk = f.name
	...
	def __new__(cls,name,bases,attrs):
		...
		for k,v in attrs.iteritems():
			if isinstance(v,Filed):
				...
				if v.primary_key:
					if primary_key:
						raise TypeError('Cannnot define more than 1 primary key in class: %s'%name)
					if v.updatable:
